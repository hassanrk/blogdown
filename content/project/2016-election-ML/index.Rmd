---
title: Sentiment Analysis on Political Tweets and Predicting Google Trends Search
  Queries
author: Hassan Rahim Kamil
date: '2019-01-06'
summary: Could we predict political outcomes with sentiments?
slug: sentiment-analysis-on-political-tweets-and-predicting-google-trends-search-queries
categories:
  - R
tags:
  - Sentiment Analysis
...
tweets = read.csv("2019-01-06-tweets.csv", stringsAsFactors = F) 

tweets_mutated = read.csv("2019-01-06-tweets_mutated.csv")
...
Microblogging is becoming more and more ubiquitous among today's generation. In fact, interactions on some type of online platform or service such as Facebook, Twitter or Google leave traces of data that show a record of behavior or actions. As Davidowitz put it in his book *Everybody Lies*: "The everyday act of typing a word or phrase into a compact, rectangular white box leaves a small trace of truth that, when multiplied by millions, eventually reveals profound realities." To statisticians, this quote faintly reminds of the many statistical ideas that could be put to the test.
...
![Figure 1: Predicting online election polls with sentiment data for Clinton.](/post/2019-01-06-sentiment-analysis-on-political-tweets-and-predicting-google-trends-search-queries_files/clinton.png)

![Figure 2: Predicting online election polls with sentiment data for Trump.](/post/2019-01-06-sentiment-analysis-on-political-tweets-and-predicting-google-trends-search-queries_files/trump.png)
...
##### **Hillary's search hits logistic diagnostic:**
...
##### **Trump's search hits logistic diagnostic:**

```{r trump logistic, echo = F, message = F, warning = F}
# Trump logistic
...
LASSO.H_plot = ggplot(data.frame("day" = rep(268:1, 2), "trend" = c(yhat111, res_H$hits)*100, "type" = c(rep("predicted", 268), rep("real", 268))), aes(x = day, y = trend, col = type )) + geom_line() + geom_point(alpha = 0.3) + ggtitle("Hillary's GTrends using LASSO") + theme_gdocs()

LASSP.T_plot = ggplot(data.frame("day" = rep(268:1, 2), "trend" = c(yhat222, res_T$hits)*100, "type" = c(rep("predicted", 268), rep("real", 268))), aes(x = day, y = trend, col = type )) + geom_line() + geom_point(alpha = 0.3) + ggtitle("Trump's GTrends using LASSO") + theme_gdocs()
...
rf.H = rf.H = randomForest(x = DTM, y = res_H$hits, ntree = 300, mtry  = 80, nodesize = 32)
rf.H.pred = predict(rf.H)
varImpPlot(rf.H, main = "Variable Importace of Hillary's RF regression")

rf.T = randomForest(x = DTM, y = res_T$hits, ntree = 300, mtry  = 77, nodesize = 37)
...
```{r RF MSE}
## MSE for Hillary's RF model (adjusted for scale)
mse(rf.H$predicted, res_H$hits)*100^2

## MSE for Trump's RF model (adjusted for scale)
mse(rf.T$predicted, res_T$hits)*100^2
```
...
We can see from the visualization which methods follow the patterns well.
